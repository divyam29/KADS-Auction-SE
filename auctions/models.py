...
class Auction(models.Model):
    ...
    current_bid = models.DecimalField(
        max_digits=9, decimal_places=2, default=0.00)
    # related_name search all the auctions with a given category
    category = models.ForeignKey(Category, on_delete=models.CASCADE,
                                 related_name="auction_category", blank=True, null=True)
...
class Watchlist(models.Model):
    auctions = models.ManyToManyField(
        Auction, related_name="auctions_in_watchlist", blank=True)
    user = models.OneToOneField(
